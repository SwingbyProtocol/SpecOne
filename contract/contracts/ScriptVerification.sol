...
contract ScriptVerification {
     ...
     */
    function redeemScriptToSecretHash(bytes memory _redeemScript) public pure returns (bytes20, bytes32) {
        bytes32 secretHash;
     ...
     * @return bytes20
     */
    function hash160(bytes memory _str) internal pure returns (bytes20) {
        return ripemd160(abi.encodePacked(sha256(_str)));
    }
