...
module.exports = async function (deployer) {
  let sv = await ScriptVerification.deployed()
  deployer.deploy(WitnessEngine).then(async() => {
    let we = await WitnessEngine.deployed()
    console.log(sv.address, we.address, we.address)
    return deployer.deploy(Burner, sv.address, we.address, we.address)
  }).then(async () => {

    const burner = await Burner.deployed()

    const burnS = await burner.submitReq(3000000, "0x6304bef4695bb175a82064b55a465fc3918f8fe5b164d049b5afa0f0926f9704957a75030ebc531604c08876a914cd5cab7ec76ad300712c02ccc51b464fdf7e5e0667040e3b6a5bb17576a9142f5e9b3a149467d002195d790ad513eac7496aa86888ac")
